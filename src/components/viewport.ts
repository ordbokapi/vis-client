...
export class Viewport extends StateManagedElement {
  ...
  connectedCallback() {
    // Wait until viewport is fully initialized
    if (!this.app || !this.viewport || !this.graphView) {
  ...
  get zoomLevel(): number {
    return this.viewport ? this.viewport.scale.x * 100 : 100;
  }
      ...
      return new Promise<void>((resolve) => {
        const interval = setInterval(() => {
          if (this.#client) {
            ...
            resolve(this.loadGraph(articleId, dictionary));
          }
        }, 100);
      });
    }
